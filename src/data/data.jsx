import productExp from "../assets/productPng/productExp.png";

const data = {
  brand: { name: "Bru7" },
  myStore: {
    phoneNumber: "(510) 999 07 18",
    email: "brunogokmen@gmail.com",
    discountLetter: "Follow Us and get a chance to win 25% off",
  },
  carousel1: {
    h2: "SUMMER 2020",
    h3: "NEW COLLECTION",
    h4: "We know how large objects will act, but things on a small scale.",
  },
  editorsPick: {
    h2: "EDITOR’S PICK",
    h3: "Problems trying to resolve the conflict between",
  },
  bestSeller: {
    h4: "Featured Products",
    h2: "BESTSELLER PRODUCTS",
    h3: "Problems trying to resolve the conflict between",
    mapArr: {
      pic: productExp,
      h5: "Graphic Design",
      h6: "English Department",
      price: "$16.48",
      priceDiscounted: "$6.48",
    },
  },
  xProduct: {
    h2: "SUMMER 2020",
    h3: "Part of the Neural Universe",
    p: "We know how large objects will act, but things on a small scale.",
  },
  blogPost: {
    h4: "Practice Advice",
    h2: "Featured Posts",
    p: "Problems trying to resolve the conflict between the two major realms of Classical physics: Newtonian mechanics",
  },
  footerArr: {
    1: ["Legal", "About Us", "Carrier", "We are hiring", "Blog"],
    2: ["Company Info", "About Us", "Carrier", "We are hiring", "Blog"],
    3: [
      "Features",
      "Business Marketing",
      "User Analytic",
      "Live Chat",
      "Unlimited Support",
    ],
    4: ["Resources", "IOS & Android", "Watch a Demo", "Customers", "API"],
    5: ["Get In Touch", "Lore imp sum dolor Amit"],
  },
  copyright: "Made With Love By Bru All Right Reserved ",
  productListPage: {
    shopCategory: {
      h3: "CLOTHS",
      p: "5 Items",
    },
  },
  aProduct: {
    h2: "Floating Phone",
    review: "10 reviews",
    price: "$1,139.33",
    availability: "Availability  :",
    isStock: "In Stock ",
    p: "Met minim Mollie non desert Alamo est sit cliquey dolor do met sent. RELIT official consequent door ENIM RELIT Mollie.Excitation venial consequent sent nostrum met.",
    blog: {
      description: "Description",
      extraInfo: "Additional Information",
      review: "Reviews",
      h4: "the quick fox jumps over ",
      p1: "Met minim Mollie non desert Alamo est sit cliquey dolor do met sent. RELIT official consequent door ENIM RELIT Mollie. Excitation venial consequent sent nostrum met.",
      p2: "Met minim Mollie non desert Alamo est sit cliquey dolor do met sent. RELIT official consequent door ENIM RELIT Mollie. Excitation venial consequent sent nostrum met.",
      p3: "Met minim Mollie non desert Alamo est sit cliquey dolor do met sent. RELIT official consequent door ENIM RELIT Mollie. Excitation venial consequent sent nostrum met.",
      hp: "the quick fox jumps over the lazy dog",
    },
  },
  pricing: {
    h3: "PRICING",
    h2: "Simple Pricing",
    p: "Problems trying to resolve the conflict between the two major realms of Classical physics: Newtonian mechanics",
    monthly: "Monthly",
    yearly: "Yearly",
    save: "Save 25%",
    usd: "$",
    pmonth: "Per Month",
    free: {
      free: "FREE",
      h4: "Organize across all apps by hand",
      f1: "Unlimited product updates",
      f2: "Unlimited product updates",
      f3: "Unlimited product updates",
      f4: "1GB Cloud storage",
      f5: "Email and community support",
      trial: "Try for free",
    },
    standard: {
      standard: "STANDARD",
      h4: "Organize across all apps by hand",
      f1: "Unlimited product updates",
      f2: "Unlimited product updates",
      f3: "Unlimited product updates",
      f4: "1GB Cloud storage",
      f5: "Email and community support",
      trial: "Try for free",
    },
    premium: {
      premium: "PREMIUM",
      h4: "Organize across all apps by hand",
      f1: "Unlimited product updates",
      f2: "Unlimited product updates",
      f3: "Unlimited product updates",
      f4: "1GB Cloud storage",
      f5: "Email and community support",
      trial: "Try for free",
    },
  },
};

export default data;
